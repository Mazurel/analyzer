...
abstract class ParsersApp : App {
  val usedColors: List<Color> = listOf(Color.BRIGHT_GREEN, Color.BRIGHT_BLUE)
  ...
  override fun run(): (Session.() -> Unit) = {
    ...
    section {
          if (inputLine == null) {
            ...
            var lines = getFileContent(inputLine!!) ?: listOf(inputLine!!)

            lines.forEachIndexed { i, line ->
              color(usedColors[i % usedColors.size]) { text(printLine(line)) }
            }
