...
fun String.padTokenTo(width: Int = 10, offset: Int = 0): String =
    this.drop(offset).drop(offset).replace('\t', ' ').take(width).padEnd(width, ' ')

fun RenderScope.showLineComparison(
    lineNumber: Int,
    baseline: LogLine,
    checked: LogLine?,
    offset: Int = 0
  ...
  scopedState {
    when {
      checked == null -> green()
      baseline.timestamp.nonTimestampString == checked.timestamp.nonTimestampString ->
          black(isBright = true)
      else -> white()
    }
...
class LogMatchingApp : App {
  ...
  override fun run(): (Session.() -> Unit) = {
    ...
    val initialPath =
        Path("../legacy-analyzer/sample-logs/").toAbsolutePath().normalize().toString()
    ...
    while (rerun) {
      ...
      if (!baselineFileContext.isValidInput() || !checkedFileContext.isValidInput()) {
        section {
            ...
            .runUntilSignal { onKeyPressed { signal() } }
        rerun = false // quit
        continue
      }
      ...
      section { textLine("$status - $progress") }
          .runUntilSignal {
            status = "Loading baseline lines"
            progress = 0
            baseline = LogFile(baselinePath.bufferedReader()) { progress += 1 }
            status = "Loading checked lines"
            progress = 0
            checked = LogFile(checkedPath.bufferedReader()) { progress += 1 }
            signal()
          }
      ...
      section {
            ...
            for (i in verticalIndex..(verticalIndex + linesAmount).min(matchingResult.size - 1)) {
              try {
                showLineComparison(
                    i, baseline!!.lineAt(i), matchingResult[i - 1], offset = horizontalIndex)
              } catch (ex: Exception) {
